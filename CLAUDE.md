...
- Don't run inference for labels **in the map pipeline**. Both the 19-class semantic label and the `moving-*` motion flag come straight from the SemanticKITTI raw `.label` files (`src/perception/semantics.py`), and the helpers that would infer them raise on purpose. Disclose plainly: the mapping contribution is evaluated independently of segmentation quality. That is the point of §9 — the variable-resolution grid is the contribution, how a point got its class is not.
- FRNet **works, and is a separate track** — do not read the rule above as "FRNet is broken". The standalone port was non-functional (~15% point accuracy) until **2 Sep 2026**; all three divergences from the checkpoint are fixed and named in `src/perception/frnet/frnet.py`'s header. It now scores **90.3% point accuracy / 65.2% mIoU over 200 frames of seq 08**, against the paper's 73.3%. 65.2% is the figure — 51.5% and 69.8% are arithmetic errors still in circulation, and 98.3% is a single-frame sanity check on seq 00 that must never be paired with a 200-frame mIoU. The model is reported **alongside** the map, never swapped into it, and fine-tuning it — such as the 600-step run in `scripts/aws/t4.sh` — is sanctioned roadmap work, not a violation of the line above.
