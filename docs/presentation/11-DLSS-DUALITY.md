...
| **Motion vectors** | 2D screen-space vectors + disocclusion mask | Ego-velocity anisotropic scaling `a_f(v)` + O(1) range-image visibility. **Correct as pitched**, with the caveat that `a_f(v)` puts the ring boundary at a non-lattice position — open item **D2 / R3**. |
